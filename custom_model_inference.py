...
class AMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(self.max_steps):
            ...
            new_flat, cert_flat = self.moe(flat)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, max_len, mlp_dim, heads, dim_head, experts, base=10000, dropout=0.):
        super().__init__()
        self.norm = nn.RMSNorm(dim)
...
class LLM(nn.Module):
    def __init__(self, dim, depth, max_len, mlp_dim, heads, dim_head,
                 vocab_size, padding_idx, experts, base=10000, dropout=0.):
        super().__init__()
