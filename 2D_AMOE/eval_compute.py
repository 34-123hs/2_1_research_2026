"""
eval_compute.py — 학습된 AMoE 체크포인트의 **효율 지표**를 val.bin(eval/가변 스텝)에서 측정.

품질(task_CE·LBL·perplexity)은 eval_custom.py가 담당한다. 여기서는 가변 스텝(eval 조기 break)에서만
의미가 있는 지표를 잰다:

    - 벽시계 시간 (time_total_s, tokens_per_s, ms_per_forward)
    - FLOPs (total, per token)  ← flops_count.py와 동일 측정(MAC×2)
    - 평균 ponder step (토큰별 기대 halt 스텝, 적응성 지표)

FLOPs 계측(FlopCounterMode)은 op를 instrument 하느라 시간을 왜곡하므로, **시간 측정과 FLOPs 측정을
서로 다른 패스로 분리**한다. 결과는 W&B(bench/* 스칼라)에 로깅된다. model.py 는 불변.

  python eval_compute.py --ckpt main_out/model.safetensors --val_bin val.bin \
      --project amoe-eval --run_name my-bench
"""
...
def main():
    ...
    # 학습과 동일하게: 각 AMoE 수직 루프 = ponder_steps, 추론이라 checkpoint off.
    # eval() 이므로 AMoE는 조기 break(가변 스텝)으로 돈다 → 효율 지표가 적응성을 반영.
    ...
    if os.path.exists(args.ckpt):
        ...
        print(f"[warn] ckpt 없음({args.ckpt}) → 랜덤 init로 측정. "
              f"halting 행동이 달라 적응 지표는 비대표적입니다.")
    ...
    avg_ponder_step = pstep_sum[0] / max(1, pstep_cnt[0])
    ...
    print(f"평균 ponder step   : {avg_ponder_step:.3f} (max {args.ponder_steps})")
    ...
    logs = {
        ...
        "bench/avg_ponder_step": avg_ponder_step,
