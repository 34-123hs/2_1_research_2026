"""
eval_custom.py — 학습된 AMoE 체크포인트(safetensors)를 val.bin에서 평가한다.

학습 loss = task_CE + ponder_beta·ponderKL + alpha·LBL 이지만,
여기서는 **ponder 항을 제외**한 loss 를 측정한다:

    eval_loss = task_CE + alpha·LBL          # ponder 제거, LBL 유지

perplexity 는 순수 task_CE 기준 exp(task_CE) 로 보고한다(LBL은 NLL이 아님).

LLM.forward 는 (loss, logits) 만 돌려주고 LBL 을 따로 노출하지 않으므로,
forward 본문을 그대로 복제하되 ponder 항만 빼서 LBL 을 직접 꺼낸다
(model.transformer(x) → (x, halting_probs, total_LBL)). model.py 는 불변.

이 파일은 **품질 지표 전담**(고정 스텝). 시간·FLOPs 등 효율 지표는 eval_compute.py 참고.
AMoE 동적 추출 때문에 task_CE·LBL 값 자체는 고정/가변 스텝에 무관하게 동일하지만,
학습 루프 의미(고정 max_steps)와 일치시키려 AMoE만 train()으로 둔다(아래 main 참고).
결과는 W&B(eval/* 스칼라)에 로깅된다.

  python eval_custom.py --ckpt main_out/model.safetensors --val_bin val.bin \
      --project amoe-eval --run_name my-eval
"""
...
def evaluate(model, loader, alpha, device):
    """
    ponder 제외 eval. LLM.forward 본문을 복제하되 ponder 항만 뺀다.
    ...
    for batch in tqdm(loader, desc="eval"):
        ...
        x, _halting, total_LBL = model.transformer(x)      # [B,block,D], list, scalar
    ...
    eval_loss = task_ce + alpha * avg_lbl                  # ponder 제외 loss
...
def main():
    ...
    # 학습과 동일하게: AMoE 수직 루프 = ponder_steps, 추론이라 checkpoint off.
    # 또한 AMoE 서브트리만 train()으로 둬 조기 break을 끄고 고정 max_steps로 돈다
    # (dropout=0이라 train()이 켜는 dropout은 no-op; evaluate가 no_grad라 checkpoint/grad 무관).
    ...
    print(f"[model] params={n_params/1e6:.2f}M  dim={args.dim} depth={args.depth} "
          f"experts={args.experts} ponder_steps={args.ponder_steps}")
    ...
    logs = {
        "eval/loss": m["eval_loss"],        # task_CE + alpha·LBL
