...
def test_compute_aux_and_metrics_integration():
    ...
    class FakeCollector:
        def __init__(self, gl, hp):
            self.gate_logits = gl
            self.halting_probs = hp
            self._gate_cap = len(gl)

    depth = 2
    gl = [torch.randn(50, 4) for _ in range(depth)]    # spl = 1
    hp = [torch.rand(5, 2, 8) for _ in range(depth)]   # T = 5
    bal, metrics, cert, table = twh.compute_aux_and_metrics(
        FakeCollector(gl, hp), depth, log_per_layer=True)
    assert bal.ndim == 0 and torch.isfinite(bal)
    assert "aux/balance_loss" in metrics
    assert "router/max_pct_global" in metrics
    assert cert.shape == (depth, 5)
