...
layer/expert/halting/grad 상태를 한 번에 진단.
...
# ============================================================================
# Part 1: 수집부 — hook으로 grad/router/halting/activation 정보 모음
# ============================================================================
class StabilityProbe:
    def __init__(self, model):
        self.model = model
        self.handles = []
        self.halting_probs = {}                   # {layer_idx: [T, B, N]}
...
def analyze(*, grad_stats, halting_probs, gate_selections, raw_certainty,
            act_norms, depth, n_experts, max_amoe_steps,
    ...
    # ---- [F] AMoE halting distribution ----
    print(f"\n[F] AMoE halting (mean step in [0, {max_amoe_steps - 1}])")
    for i in range(depth):
        hp = halting_probs[i].float()
        T = hp.size(0)
        steps = torch.arange(T, device=hp.device, dtype=hp.dtype).view(T, 1, 1)
        mean_step = (hp * steps).sum(dim=0).mean().item()
        coverage = hp.sum(dim=0).mean().item()
        if mean_step < 0.5:
            flag = "⚠️ 즉시 halt (AMoE 비활성)"
        elif mean_step > max_amoe_steps - 1.5:
            flag = "⚠️ 항상 max — halting 학습 안 됨"
        else:
            flag = "✅"
        print(f"  L{i:2d}: mean_step={mean_step:.2f}  coverage={coverage:.3f}  [{flag}]")
    ...
    # ---- [H] Per-token expert + raw certainty dump ----
    if do_dump:
        n_show = min(dump_tokens, block_size)
        print(f"\n[H] Per-token expert + raw certainty "
              f"(b=0, first {n_show} of {block_size} tokens)")
        for i in range(depth):
            sels = gate_selections[i]
            certs = raw_certainty[i]
            for t, (sel, cert) in enumerate(zip(sels, certs)):
                sel_b0 = sel.view(batch_size, block_size)[0, :n_show].tolist()
                cert_b0 = cert.view(batch_size, block_size)[0, :n_show].tolist()
                row = " ".join(f"E{e}({c:.2f})" for e, c in zip(sel_b0, cert_b0))
...
def main():
    ...
    max_amoe_steps = model.transformer.layers[0][1].max_steps
    probe.close()

    analyze(
        grad_stats=grad_stats,
        halting_probs=probe.halting_probs,
        gate_selections=probe.gate_selections,
        raw_certainty=probe.raw_certainty,
        act_norms=probe.act_norms,
        depth=args.depth,
        n_experts=args.experts,
        max_amoe_steps=max_amoe_steps,
